  ...
  return (tree) => {
    visit(tree, "element", (node: Element) => {
      ...
      if (
        ...
        // @ts-expect-error TS2339 it freaks out because number does not support `includes`
      ...
      try {
        ...
        node.children.unshift({
          type: "comment",
          value: `Failed to process mermaid diagram: ${error instanceof Error ? error.message : "Unknown error"}`,
        })
      }
    })

...
async function getChaptersByNamespace(namespace: string) {
  return allChapters.filter((chapter) => chapter.namespace === namespace).sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0));
}
...
export const TableOfContents = async ({ namespace, className }: Props) => {
  ...
  return (
    <>
      <TableOfContentsPopover course={course} chapters={chapters} className="max-sm:hidden" />
      <TableOfContentsDrawer course={course} chapters={chapters} className="sm:hidden" />
    </>
  )
}

...
export const Quiz = ({
  ...
  hero = {
    ...
    description:
      "Проверьте свои знания и посмотрите, чему вы только что научились.",
  },
}: Props) => {
  ...
  if (!mounted) {
    return <QuizRoot className="min-h-[640px] w-full" />
  }
  ...
  return (
    <QuizRoot>
      ...
      <QuizContent>
        ...
        <QuizOptions
          className={cn({
            "flex w-full flex-1 flex-col items-center justify-center rounded-lg border p-8":
              status !== "idle",
          })}
        >
          {status === "idle" && (
        ...
        <QuizFooter
          className={cn({ "justify-center": status === "incorrect" })}
        >
          {status === "idle" && (
